...
class GoogleCalendarParser:
    ...
    def __get_event_from_google_calendar(self) -> Union[dict, None]:
        try:
    ...
    def get_event(self) -> Union[dict, None]:
        event = self.__get_event_from_google_calendar()

        staff_email = event.get("creator").get("email")
        start_time = self.__normalize_datetime(event.get('start').get('dateTime'))
        if start_time != self.__get_current_time_str() or staff_email not in STAFF_INFO:
            return
    ...
    @staticmethod
    def __get_current_time_str() -> str:
